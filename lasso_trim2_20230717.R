...
## Make a recipe
data_recipe <- recipe(anomalies ~ ., data = analysis_train[c(1, 40:59)]) %>%
...
lasso_fit_trim2 <- final_wf %>% fit(data = analysis_train[c(1, 40:59)])
...
anomalieslasso_pred <- predict(lasso_fit_trim2, analysis_train[c(1, 40:59)], type = "prob") %>%
  bind_cols(analysis_train[c(1, 40:59)] %>% select(anomalies))

# Print outcomes for the first 10 people
head(anomalieslasso_pred, n = 10)

# Plot the prediction
Pred_plot_lasso_trim2 <- anomalieslasso_pred %>%
  ggplot() +
  geom_density(
...
print(Pred_plot_lasso_trim2)


# Plot ROC curve for the Lasso model on the training data
roc_lasso_plot_trim2 <- anomalieslasso_pred %>%
  roc_curve(truth = anomalies, .pred_0) %>%
  autoplot()


roc_lasso_plot_trim2
...
lasso_matrices_trim2 <- collect_metrics(last_lasso_fit_trim2, summarize = TRUE)

# Generate confusion matrix
lasso_conf_mat_trim2 <- last_lasso_fit_trim2 %>%
  collect_predictions() %>%
...
vip_lasso_trim2 <- lasso_fit_trim2 %>%
  extract_fit_parsnip() %>%
  vip(num_features = 20)
...
saveRDS(vip_lasso_trim2, "./data/vip_lasso_trim2.rds")
saveRDS(Pred_plot_lasso_trim2, file = "./data/Pred_plot_lasso_trim2.rds")
saveRDS(roc_lasso_plot_trim2, file = "./data/roc_lasso_plot_trim2.rds")
saveRDS(lasso_matrices_trim2, file = "./data/lasso_matrices_trim2.rds")
saveRDS(lasso_conf_mat_trim2, file = "./data/lasso_conf_mat_trim2.rds")

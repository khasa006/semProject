...
## Make a recipe
data_recipe <- recipe(anomalies ~ ., data = analysis_train[c(1, 21:39, 59)]) %>%
...
lasso_fit_trim1 <- final_wf %>% fit(data = analysis_train[c(1, 21:39, 59)])
...
anomalieslasso_pred <- predict(lasso_fit_trim1, analysis_train[c(1, 21:39, 59)], type = "prob") %>%
  bind_cols(analysis_train[c(1, 21:39, 59)] %>% select(anomalies))

# Print outcomes for the first 10 people
head(anomalieslasso_pred, n = 10)

# Plot the prediction
Pred_plot_lasso_trim1 <- anomalieslasso_pred %>%
  ggplot() +
  geom_density(
...
print(Pred_plot_lasso_trim1)


# Plot ROC curve for the Lasso model on the training data
roc_lasso_plot_trim1 <- anomalieslasso_pred %>%
  roc_curve(truth = anomalies, .pred_0) %>%
  autoplot()


roc_lasso_plot_trim1
...
lasso_matrices_trim1 <- collect_metrics(last_lasso_fit_trim1, summarize = TRUE)

# Generate confusion matrix
lasso_conf_mat_trim1 <- last_lasso_fit_trim1 %>%
  collect_predictions() %>%
...
vip_lasso_trim1 <- lasso_fit_trim1 %>%
  extract_fit_parsnip() %>%
  vip(num_features = 20)
...
saveRDS(vip_lasso_trim1, "./data/vip_lasso_trim1.rds")
saveRDS(Pred_plot_lasso_trim1, file = "./data/Pred_plot_lasso_trim1.rds")
saveRDS(roc_lasso_plot_trim1, file = "./data/roc_lasso_plot_trim1.rds")
saveRDS(lasso_matrices_trim1, file = "./data/lasso_matrices_trim1.rds")
saveRDS(lasso_conf_mat_trim1, file = "./data/lasso_conf_mat_trim1.rds")
